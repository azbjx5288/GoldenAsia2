...
public class GameHistoryAdapter extends BaseAdapter
{
    ...
    /*private void setPackageData(ViewHolder viewHolder, PackageBean packageBean)
    {
        ...
        Lottery lottery = userCentre.getLottery(Integer.parseInt(packageBean.getLottery_id()));
        if (lottery != null)
        {
            viewHolder.name.setText(lottery.getCname());
        } else
        ...
        switch (Integer.parseInt(packageBean.getCheck_prize_status()))
        {
            case 0:
                viewHolder.prize.setVisibility(View.GONE);
                viewHolder.state.setTextColor(colors[STATE_DOING]);
                viewHolder.state.setText(states.get(Integer.parseInt(packageBean.getCancel_status())));
                break;
