...
public class RestRequest implements Response.ErrorListener, Response.Listener<RestResponse> {
    ...
    public GsonRequest createGsonRequest(Type type) {
        ...
        request.setRetryPolicy(new DefaultRetryPolicy((int)(10 * ONE_SECOND), 3, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        return request;
    }
    ...
    private void handleErrorDefault() {
        if (errCode == 7001) { //用户没有登录，其值为服务器定义的
            if (handleNotLogin()) {
                return;
            }
        }
    ...
    private boolean handleNotLogin() {
        ...
        LoginCommand command = GoldenAsiaApp.getUserCentre().createLoginCommand();
        if (command == null) {
            //不能自动登录
            return false;
        }
        ...
        RestRequestManager.executeCommand(context, command, new RestCallback() {
            ...
            public boolean onRestComplete(RestRequest request, RestResponse response) {
                ...
                RestRequest.this.request.setRetryPolicy(new DefaultRetryPolicy((int)(10 * ONE_SECOND), 3, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
                execute();
                return false;
            }
            ...
            public boolean onRestError(RestRequest request, int errCode, String errDesc) {
                ...
                Toast.makeText(context, /*"出错了：[ " +*/ RestRequest.this.errCode + ", " + RestRequest.this.errDesc /*+ " ]"*/,
