...
public abstract class Game implements NumberGroupView.OnChooseItemClickListener, DigitCheckBoxPanel
    ...
    public final void initDigitPanel(View view, int digit) {
        digitCheckBoxPanel = new DigitCheckBoxPanel(getTopLayout().getContext(), view.findViewById(R.id.digit),method, digit);
    ...
    protected static String transformtextSpecial(ArrayList<Integer> list, String[] disText, boolean numberStyle,boolean emptyStyle) {
        StringBuilder builder = new StringBuilder();
        if (list.size() > 0) {
            for (int i = 0, size = list.size(); i < size; i++) {
                builder.append(disText[list.get(i) - 1]);
