...
public class DrawTrendView extends View {
    ...
    private Paint rowForPaint;
    private Paint rowForDefaultPaint;
    private int linkBallLineColor;
    private int rowForColor;
    private int rowForColorDefault;
    ...
    private void init(AttributeSet attrs) {
        ...
        linkBallLineColor = attribute.getColor(R.styleable.TrendView_tr_linkBallLineColor, Color.GRAY);

        rowForColor = attribute.getColor(R.styleable.TrendView_tr_rowForColor, Color.GRAY);
        rowForColorDefault = attribute.getColor(R.styleable.TrendView_tr_rowForColorDefault, Color.GRAY);

        attribute.recycle();
        this.linePaint = new Paint();
        this.rowForPaint =new Paint();
        this.rowForDefaultPaint =new Paint();
    ...
    public void setLinkBallLineColor(int linkBallLineColor) {
        this.linkBallLineColor = linkBallLineColor;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawXLine(canvas);
        drawYLine(canvas);
        drawDivideBackdrop(canvas);
        drawRightVerticalLine(canvas);
        if (isNeedLinkLine()&&!(morelinkData != null && morelinkData.length > 0)) {
    ...
    private void drawDivideBackdrop(Canvas canvas){
        rowForPaint.setColor(rowForColor);
        rowForPaint.setStyle(Paint.Style.FILL);//充满
        rowForDefaultPaint.setColor(rowForColorDefault);
        rowForDefaultPaint.setStyle(Paint.Style.FILL);//充满
        if (solelinkData != null && solelinkData.length > 0) {
            for (int i = 0; i < solelinkData.length; i++) {
                for (int j = 0, count = maxcopiesx - mincopiesx + 1; j < count; j++) {
                    float[] xyRow = this.translateRowIndex2XY(i, j);
                    if (i % 2 == 0){
                        canvas.drawRect((int) xyRow[0], (int) xyRow[1], (int) xyRow[0] + itemSize, (int) xyRow[1] + itemSize, rowForPaint);
                    }else{
                        canvas.drawRect((int) xyRow[0], (int) xyRow[1], (int) xyRow[0] + itemSize, (int) xyRow[1] + itemSize, rowForDefaultPaint);
                    }
                }
            }
        } else if (morelinkData != null && morelinkData.length > 0) {
            canvas.drawColor(rowForColorDefault);
        }
    }
    ...
    private void more(Canvas canvas) {
        ...
        for (int i = 0; i < morelinkData.length; i++) {
            ...
            for (int j = 0; j < testRow.length; j++) {
                ...
                textX = xy[0] + 0.5f * (itemSize - linePaint.measureText(testRow[j]));
                textY = xy[1] + 0.5f * itemSize + 0.25f * linePaint.getTextSize();
                if (numberStyle == null) {
                    if (displayText.length >= maxcopiesx) {
                        text = moreDisplay(i);
