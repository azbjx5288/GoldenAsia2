...
public class DrawCodeView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        int measuredHeight = measureHeight((verticalGap + itemSize) * copies_y);
        ...
        for (int i = 0; i < morelinkData.length; i++) {
            ...
            for (int j = 0; j < testRow.length; j++) {
                ...
                if (testRow.length > 1) {
                    ...
                    linePaint.setTextSize(textSize - 2);//比顶部字体小点
                    checkedDrawable.setBounds((int) xy[0], (int) xy[1], (int) (xy[0] + this.delt_x), (int) (xy[1] + this.delt_y));
                    checkedDrawable.draw(canvas);
                    textX = xy[0] + (this.delt_x / 2 - linePaint.measureText(text.isEmpty()?"x":text) / 2);
                    textY = xy[1] + (int) (this.delt_y + linePaint.getTextSize()) / 2;
                } else {
                    linePaint.setColor(Color.GRAY);
                    linePaint.setTextSize(textSize - 2);//比顶部字体小点
                    background.setBounds((int) xy[0], (int) xy[1], (int) (xy[0] + this.delt_x * 2), (int) (xy[1] + this.delt_y));
                    background.draw(canvas);
                    textX = xy[0] + (this.delt_x - linePaint.measureText(text.isEmpty()?"x":text) / 2);
                    textY = xy[1] + (int) (this.delt_y + linePaint.getTextSize()) / 2;
                }
                canvas.drawText(text.isEmpty()?"x":text, textX, textY, linePaint);
            }
        }
    }
