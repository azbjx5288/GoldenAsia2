...
public class MmcWinHistoryDao {
    private Dao<MmcWinHistory, Integer> dao;

    public MmcWinHistoryDao(Context context) {
        DatabaseHelper dbHelper = OpenHelperManager.getHelper(context, DatabaseHelper.class);
        try {
            dao = dbHelper.getDao(MmcWinHistory.class);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }

    public void savaMmcWinHistory(MmcWinHistory bean) {
        try {
    ...
    public List<MmcWinHistory> getAllMmcWinHistory() {
        try {
    ...
    public void delete(MmcWinHistory mmcWinHistory) {
     ...
     * @return 当前页的记录
     */
    public List<MmcWinHistory> getAllItems(int currentPage, int pageSize) {
        long firstResult = (currentPage - 1) * pageSize;
        long maxResult = currentPage * pageSize;
        List<MmcWinHistory> allMmcWinHistory=null;
        try {
