...
public class Calculation {
    ...
    public int calculationNote(List<String> oneNums, List<String> twoNums,
                               List<String> threeNums, List<String> assembleNums,
                               Method methodkey) {
        Log.i(TAG,"玩法->"+methodkey.getName());
        int redC = 0;
        switch (methodkey.getName()) {
            case "LTZX3":
                List<String> aIntersectsB = intersects(oneNums, twoNums);
                List<String> aIntersectsC = intersects(oneNums, threeNums);
                List<String> bIntersectsC = intersects(twoNums, threeNums);
                List<String> abc = intersects(oneNums, bIntersectsC);
     ...
     * @return
     */
    public int calculationNote(ArrayList<String> wan, ArrayList<String> qian, ArrayList<String> bai, ArrayList<String> shi, ArrayList<Integer> ge,
                               ArrayList<String> assemble,
                               ArrayList<String> single,
                               ArrayList<String> cipher,
                               ArrayList<String> unusualBai, ArrayList<String> unusualShi, ArrayList<String> unusualGe, String methodkey) {
        int redC = 0;
        switch (methodkey) {
            case "YXZX":    //后一直选
                redC = ge.size();
            ...
            case "QSIZX":   //前四直选
                ...
                break;
            case "ZUX4":    //后四组选4
                List<String> intersects4 = intersects(assemble, single);
            ...
            case "ZUX24":   //后四组选24
                ...
                break;
            case "ZUX12":   //后四组选12
                List<String> intersects12 = intersects(assemble, single);
            ...
            case "ZUX120":  //组选120
                ...
                break;
            case "ZUX5":    //组选5
                List<String> intersects5 = intersects(assemble, single);
                redC = C(assemble.size(), 1) * C(single.size(), 1) - C(intersects5.size(), 1);
                break;
            case "ZUX60":   //组选60
                List<String> intersects60 = intersects(assemble, single);
            ...
            case "WXZX":    //五星直选
                ...
                break;
            case "ZUX30":   //组选30
                List<String> intersects30 = intersects(assemble, single);
            ...
            case "WXLX":    //五星连选
                ...
                break;
            case "ZUX20":   //组选20
                List<String> intersects20 = intersects(assemble, single);
                redC = C(assemble.size(), 1) * C(single.size(), 2) - C(intersects20.size(), 1) * C(single.size() - 1, 1);
                break;
            case "ZUX10":   //组选10
                List<String> intersects10 = intersects(assemble, single);
    ...
    // 交集
    private List<String> intersects(List<String> arrayFirst, List<String> arraySecond) {
        List<String> intersectSet = new ArrayList<>(arrayFirst);
        intersectSet.retainAll(arraySecond);
        return intersectSet;
    }

    private int methodJiaoji(List<String> arrayFirst, List<String> arraySecond) {
    ...
    private enum Bits {
        ...
        // 普通方法
        public static int getName(ArrayList<String> name, String place, String types) {
            int estimate = 0;
            for (String n : name) {
                    ...
                    switch (a[0]) {
                        case "Two":
                            if (a[0].equals(place) && a[1].equals(types)) {
                                if (c.getName() == Integer.parseInt(n)) {
                                    estimate += c.index;
                                }
                            }
                            break;
