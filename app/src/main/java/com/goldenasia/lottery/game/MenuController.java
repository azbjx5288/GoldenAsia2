  ...
  /*  @Bind(R.id.game_preference_1) TextView preference1;
    ...
    private ChooserModel getChooserModel() {
        if (chooserModel == null) {
            chooserModel = ChooserModel.get(activity, "model_history_" + GoldenAsiaApp.getUserCentre().getUserID() + "_" + lotteryId);
        }
        return chooserModel;
    }
        ...
        for (int i = 0; i < tags.size(); i++) {
            if (map.get(i)) {
                ...
                if (onClickMethodListener != null) {
                    onClickMethodListener.onClickMethod(chooserModel.getMethodInfo(i).method);
                }
            }
        }
    }
    ...
    private void usePreference() {
        ...
        for (Integer in : map.keySet()) {
            if (currentMethod != null && currentMethod.getMethodId() == chooserModel.getMethodInfo(in).method.getMethodId()) {
                common = in;
            }
        }
