...
public class ShoppingMmcFragment extends BaseFragment {
    ...
    public void planPrompt() {
        chooseTips.setTipsText(String.format("%d", cart.getPlanNotes()), String.format("%.3f", cart.getPlanAmount()), String.format("%.3f", userCentre.getUserInfo().getBalance()));
    }
        ...
        chaseMmcLottery.setOnChaseStartListener((int openCount, int[] traceData, boolean winStop, double totalCost) -> {
            ...
            if (userCentre.isLogin()) {
                ...
                if (totalCost <= userCentre.getUserInfo().getBalance()) {
                    ...
                    payMmcCommand.setPrizeMode(cart.getPrizeMode()>0?1:0/*!(userCentre.getBonusMode(lottery.getLotteryId()) == 0)*/);
            ...
            public void run() {
                ...
                if (rollCount < length) {
                    ...
                    if (isInTraceState == true) {
                        RollCountPopupWindow rollCountPopupWindow = new RollCountPopupWindow(getActivity(), rollCount + 1);
